...
class Product:
    ...
    def explore(self):
        while(True):
            ...
            if pdt==1:
                self.display(list(filter(lambda p_list: p_list['category']=='mobile',products)))
            elif pdt==2:
                self.display(list(filter(lambda p_list: p_list['category']=="washing-machine",products)))
            elif pdt==3:
                self.display(list(filter(lambda p_list: p_list['category']=='tv',products)))
            else:
                self.display(products)
            while(True):
                    operation=int(input("1. Add product\n2. Delete Product\n"))
                    if operation==1:
                        self.add(customer_list)
    ...
    def add(self,*arg,category=None):
        if(category!=None):
            productdetails={}
            ...
            products.append({'id':len(products)+1,'name': name,'product_details':productdetails ,'cost': cost,'currency': currency,'category': category,'colour': color})
            print("Product added!\n")
        else:
            while(True):
...
products=[{'id': 1,'name': "Iphone 11",'product_details': {'ram': "2 GB",'processor': "snapdragon",'screen_size': "4 inch"},'cost': "50000",'currency': "INR",'category': "mobile",'colour': "black"},
           {'id': 2,'name': "Samsung Galaxy",'product_details': {'ram': "2 GB",'processor': "snapdragon",'screen_size': "4 inch"},'cost': "45000",'currency': "INR",'category': "mobile",'colour': "grey"},
           {'id': 3,'name': "Washing Machine",'product_details': {'machine_capacity': "6kg",'machine_rpm': "50",'type': "top_load"},'cost': "25000",'currency': "INR",'category': "washing-machine",'colour': "blue"}
            ]
        ...
        while(True):
            ...
            while(True):
                if choice==1:
                    ...
                    if category==1:
                        admin.add('ram','processor','screen size',category='mobile')
                    elif category==2:
                        admin.add('machine capacity','machine rpm','machine type',category='washing-machine')
                    elif category==3:
                        admin.add('','resolution','screen size',category='tv')
                    elif category==0:
                        break
                elif choice==2:
                    admin.display(products)
